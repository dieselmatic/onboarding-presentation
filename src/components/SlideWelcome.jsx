...
export default function SlideWelcome() {
  ...
  return (
    <div className="h-screen w-screen bg-brand-black flex items-center justify-center relative overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] lg:w-[600px] lg:h-[600px] rounded-full bg-brand-orange/5 blur-[120px] animate-pulse" />
      <motion.div variants={container} initial="hidden" animate="show" className="text-center z-10 px-8">
        <motion.img variants={fadeUp} src={whiteLogo} alt="Dieselmatic" className="w-48 lg:w-72 mx-auto mb-6 lg:mb-12" />
        <motion.h1 variants={fadeUp} className="text-3xl sm:text-4xl lg:text-6xl font-headline font-extrabold italic text-brand-white mb-6">
          Welcome to Dieselmatic,{' '}<span className="text-brand-orange">{companyName}</span>
        </motion.h1>
        <motion.p variants={fadeUp} className="text-base sm:text-xl lg:text-2xl text-brand-white/70 font-body mb-8 lg:mb-16">
